...
class TestLoginGate:
    ...
    async def test_password_alone_does_not_produce_a_session(self, client, db, user):
        ...
        response = await sign_in(client, user.email)
        assert response.status_code == 200
        # A step, not a failure: the password was right.
        assert response.json()["data"]["two_factor_required"] is True
        assert response.json()["data"]["authenticated"] is False

        # And the challenge is not a session: the dashboard is still shut.
        dashboard = await client.get("/api/internal/dashboard")
    ...
    async def test_the_code_completes_the_sign_in(self, client, db, user):
        ...
        assert (await client.get("/api/internal/dashboard")).status_code == 200
    ...
    async def test_a_wrong_code_is_refused(self, client, db, user):
        ...
        response = await client.post(
            "/api/internal/auth/login/2fa",
            json={"code": "000000", "next": "/dashboard"},
            headers={"X-CSRF-Token": csrf},
        )
    ...
    async def test_the_second_step_cannot_be_reached_without_the_first(self, client, db, user):
        ...
        response = await client.post(
            "/api/internal/auth/login/2fa",
            json={"code": totp.current_code(secret), "next": "/dashboard"},
            headers={"X-CSRF-Token": csrf},
        )
        # 401 rather than the form route's 400: there is no challenge, so this
        # is an authentication failure and not a malformed request. What has to
        # hold either way is that a correct code with no preceding password
        # gets nothing.
        assert response.status_code == 401
        assert (await client.get("/api/internal/dashboard")).status_code == 401
