  ...
  return <section className="analysis-story" id="analysis" ref={ref} data-analysis-story aria-labelledby="analysis-heading">
    <div className="analysis-stage" data-analysis-stage>
      ...
      <div className="analysis-stage__body">
        <div className="analysis-stage__copy"><span className="analysis-stage__counter" key={stage}>{current.count ?? <ArrowDownRight size={60} aria-hidden="true" />}</span><p className="eyebrow">{current.unit}</p><h2 id="analysis-heading">{current.title}</h2><p>{current.copy}</p></div>
        <DependencyField storyRef={ref} progress={progress} />
      </div>
