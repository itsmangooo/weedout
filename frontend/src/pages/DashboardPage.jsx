...
export function DashboardPage() {
  ...
  return <PageFrame className="dashboard-page" eyebrow="Workspace / dependency intelligence" title={directory ? "Projects" : "Security overview"} description={directory ? "Every project, its latest check, and the findings that need attention." : "What needs your attention now, what Weedout filtered out, and where each project stands."} actions={<Link className="button button--primary" to="/targets/new"><Plus size={16} aria-hidden="true" />Add project</Link>}>
    {!directory && <><div className="workspace-live" role="status"><Radio size={13} aria-hidden="true" />{live === "live" ? "Live updates" : live === "reconnecting" ? "Reconnecting" : live === "unavailable" ? "Live updates unavailable" : "Connecting live updates"}</div><dl className="metric-strip"><DataMetric label="Needs attention" value={summary.open_findings} tone={summary.open_findings ? "warning" : "neutral"} /><DataMetric label="Critical" value={summary.critical_findings} tone={summary.critical_findings ? "critical" : "neutral"} /><DataMetric label="Known exploited" value={summary.exploited_findings} tone={summary.exploited_findings ? "critical" : "neutral"} /><DataMetric label="Noise filtered" value={`${summary.filter_rate_percent}%`} /><DataMetric label="Dependencies" value={summary.dependencies} /><DataMetric label="Projects" value={summary.projects} /></dl>
    ...
    {directory && (projects.length ? <ProjectsDirectory projects={projects} /> : <DashboardEmptyState />)}
  </PageFrame>;
}
...
function RecentChecks({ projects }) {
  ...
  return <section className="recent-checks" aria-labelledby="recent-checks-heading"><div className="recent-checks__heading"><div><p className="section-label">Latest scan state</p><h2 id="recent-checks-heading">Recent project checks</h2></div><Link to="/dashboard?view=projects">All projects <ArrowRight size={14} aria-hidden="true" /></Link></div><div>{recent.map((project) => <ProjectRow project={project} key={project.id} />)}</div></section>;
}
