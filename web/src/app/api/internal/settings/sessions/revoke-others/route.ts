...
import { revokeOtherSessions } from "@/server/repositories/account-actions";
export async function POST(request: NextRequest) { const csrf=requireCsrf(request);if(csrf)return csrf;const auth=await authenticated(request);if("response" in auth)return auth.response;try{return privateResponse({data:{revoked:await revokeOtherSessions(auth.user.id,auth.token)}},request);}catch(error){return accountActionFailure(error,request);} }
